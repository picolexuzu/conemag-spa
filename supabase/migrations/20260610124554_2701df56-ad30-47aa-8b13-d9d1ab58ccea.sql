...
-- Trigger function: enqueue notification email when a new lead is inserted
...
DROP TRIGGER IF EXISTS trg_notify_lead_email ON public.leads;
CREATE TRIGGER trg_notify_lead_email
  AFTER INSERT ON public.leads
  FOR EACH ROW EXECUTE FUNCTION public.notify_lead_email();

-- Repoint email-queue cron to new Supabase Edge Function
DO $$
BEGIN
...
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    url := 'https://ibtbeymyucvrnfgfvrpd.supabase.co/functions/v1/process-email-queue',
