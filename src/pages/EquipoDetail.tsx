...
export default function EquipmentDetailPage() {
  ...
  const related = getEquipmentList(locale)
    .filter((e) => e.slug !== eq.slug)
    .slice(0, 3);
          ...
          <div className="mt-6 grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                <a
                  href={whatsappUrl(
                    `Olá! Tenho interesse no equipamento ${eq.code} - ${eq.name}. Gostaria de receber um orçamento.`,
                  )}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 rounded-full bg-gold text-gold-foreground px-7 py-3.5 font-semibold hover:shadow-gold-glow transition"
                >
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {eq.applications.map((app) => (
              <div
                key={app}
                className="p-6 rounded-2xl bg-card border border-border hover:border-primary/40 transition"
              >
            ...
            <div className="mt-10 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {variants.map((v) => (
                <a
                  key={v}
                  href={whatsappUrl(
                    `Olá! Tenho interesse no equipamento ${eq.code} - ${v}. Gostaria de receber um orçamento.`,
                  )}
                  target="_blank"
                  rel="noopener noreferrer"
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <h3 className="mt-2 text-xl font-display font-bold">
                    {r.code}{" "}
                    <span className="text-muted-foreground font-normal text-sm">— {r.name}</span>
                  </h3>
                </div>
              </Link>
            ))}
      ...
      <section className="py-24 bg-primary text-primary-foreground relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 text-center relative">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-gold text-gold-foreground px-8 py-4 font-semibold hover:shadow-gold-glow transition"
          >
